...
Read [../AGENTS.md](../AGENTS.md) first. The root file contains the project-wide product boundaries, dependency policy, and verification policy. This file adds frontend-specific conventions.
...
This is not a Next.js application. Do not introduce Next.js, SSR, server components, file-based routing, or a Node application server. The stack is locked unless a change is explicitly approved.

## Layout

Keep the interface in this layout:
...
Keep all documented frontend checks green:
...
Do not add Vitest, Jest, Playwright, Cypress, `*.test.*`, or another automated frontend test setup. This project uses strict typing, linting, production builds, and manual browser verification as defined by the root instructions.
