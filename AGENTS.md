...
- Do not add automated test suites, `tests/` directories, or `*.test.*` files.
- Keep verification proportional: lint the backend, type-check/lint/build the frontend, exercise the fictional corpus evaluators when cloud usage is intended, and manually walk through the user story in the browser.
