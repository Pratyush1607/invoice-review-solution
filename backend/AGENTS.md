...
Read [../AGENTS.md](../AGENTS.md) first. The root file contains the project-wide product boundaries, dependency policy, and verification policy. This file adds backend-specific conventions.
...
The stack is locked unless a change is explicitly approved.

## Layout

Keep implementation inside these boundaries:
...
Do not create empty architectural layers before they are needed.
...
- Fixed application policy belongs in its immutable application configuration, not environment variables.
...
Keep the documented backend check green:
...
Do not add `tests/`, `pytest`, or committed automated test files. This project uses linting, explicit provider/corpus checks, and manual workflow verification as defined by the root instructions.
