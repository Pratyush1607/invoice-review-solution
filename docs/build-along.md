...
This guide records how the Invoice Review application was built, checkpoint by checkpoint.
...
Continue with classification, extraction, validation, and the review UI in the sections below.
...
Stage JSON columns remain for inspection; the interactive UI reads `review_data`, `issues`, `document_review`, and `accounting_coding`.
