...
Stage columns keep each pipeline step visible (`classification`, `extraction`, `validation`, `gl_suggestion`). The editable review projection is stored as `review_data`, with `document_review`, `accounting_coding`, and `issues` for Maya’s decision loop.
